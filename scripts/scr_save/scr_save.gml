function scr_save(save_part, save_id, autosaving = false) {
    ...
    var t1 = get_timer();
    try {
        LOGGER.info($"Saving to slot {save_id} - started! Autosave? {autosaving}");
        if (save_part == 1 || autosaving) {
            LOGGER.info($"Saving to slot {save_id} - part {save_part} started!");
            ...
            LOGGER.info($"Saving to slot {save_id} - vars are assigned!");
            ...
            LOGGER.info($"Saving to slot {save_id} - GameSave struct created!");
            ...
            LOGGER.info($"Saving to slot {save_id} - stars are serialized and stored!");
        }
        ...
        if (save_part == 2 || autosaving) {
            ...
            LOGGER.info($"Saving to slot {save_id} - player fleets are serialized and stored!");
            ...
            LOGGER.info($"Saving to slot {save_id} - enemy fleets are serialized and stored!");
        }
        ...
        if (save_part == 3 || autosaving) {
            ...
            LOGGER.info($"Saving to slot {save_id} - obj_controller is serialized and stored!");
        }
        ...
        if (save_part == 4 || autosaving) {
            ...
            LOGGER.info($"Saving to slot {save_id} - obj_ini is serialized and stored!");
        }
        ...
        if (save_part == 5 || autosaving) {
            ...
            ini_close();
            LOGGER.info($"Saving to slot {save_id} - saves.ini saving complete!");
            ...
            if (!autosaving) {
                LOGGER.info($"Saving to slot {save_id} - GameSave struct conversion complete!");
            } else {
                LOGGER.info($"Saving to autosave slot - GameSave struct conversion complete!");
            }
        }
    } catch (_exception) {
    ...
    if (!autosaving) {
        LOGGER.info($"Saving part {save_part} took {diff} seconds!");
    } else {
        LOGGER.info($"Autosaving took {diff} seconds!");
    }
}
