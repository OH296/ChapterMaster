...
function add_new_gene_slave() {
    if ((gene_seed > 0) && (obj_ini.zygote == 0)) {
        var _added = false;
...
function scr_apothecarium() {
    ...
    if (training_apothecary == 0) {
        blurp += "Our Brothers are currently not assigned to train further " + string(obj_ini.role[100][15]) + "; no more can be trained until Apothcarium funds are increased.";
    }
    //
    if (training_apothecary > 0) {
    ...
    if (gene_seed <= 0) {
        blurp += "##My lord, our stocks of gene-seed are empty.  It would be best to have some come mechanicus tithe.##Further training of Neophytes is halted until our stocks replenish.";
    }
    if ((gene_seed > 0) && (gene_seed <= 10)) {
        blurp += "##My Brother " + string(obj_ini.role[100][15]) + "s assigned to the gene-vault have informed me that our stocks are nearly gone.  They only number " + string(gene_seed) + "; this includes those recently recovered from our fallen comerades-in-arms.";
    }
    if (gene_seed > 10) {
        blurp += "##My Brother " + string(obj_ini.role[100][15]) + "s assigned to the gene-vault have informed me that our stocks of gene-seed currently number " + string(gene_seed) + ".  This includes those recently recovered from our fallen comerades-in-arms.";
    }
    if (gene_seed > 0) {
        blurp += "##The stocks are stable and show no sign of mutation.";
    }
    ...
    if (menu_adept == 1) {
        ...
        if (training_apothecary >= 0 && training_apothecary <= 6) {
            blurp += _recruit_pace[training_apothecary];
        }
        if (training_apothecary > 0) {
            blurp += "  The next " + string(obj_ini.role[100][15]) + " is expected in " + string(eta) + " months.";
        }
    ...
    if (obj_ini.zygote == 1) {
        blurp2 = "Unfortunantly we cannot make use of Test-Slave Incubators.  Due to our missing Zygote any use of gestation pods is ultimately useless- no new gene-seed may be grown, no matter how long we wait.";
    }
